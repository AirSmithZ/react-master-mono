...
// import Tabs, { PureTab } from '../../pages/home/tabs';
type Props = {
    hide?: Boolean;
}
    ...
    return <div>
        {
            navs?.map((item) => <NavLink
                ...
                to={item.path || ''}      
                className={({ isActive }) => {
                    return " hover:text-black mx-4 h-full py-3 transition-all " + 
...
function Navigation({ hide }: Props) {
  return (
    <div className='bg-white w-screen shadow overflow-hidden h-13 top-0 sticky box-border'>
        <div className=' max-w-7xl min-w-6xl mx-auto my-0 flex justify-center'>
            <div className={`w-full flex flex-col items-center justify-between min-w-max relative transition-all duration-300 ${hide? "top-0": "-top-13"}`}>
